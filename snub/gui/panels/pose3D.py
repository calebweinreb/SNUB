...
class Pose3DPanel(Panel, HeaderMixin):
    def __init__(self, config, data_path=None, labels_path=None, intervals_path=None,
                 joint_colors_path=None, link_colors_path=None, links_path=None, 
                 joint_size=10, link_width=5, scaling=True,
                 floor_bounds=None, floor_height=0, floor_spacing=10, floor_color=(1,1,1),
                  **kwargs):
        ...
        if links_path is None: self.link_indexes = np.zeros((0,2),dtype=int)
        else: self.link_indexes = np.load(os.path.join(config['project_directory'],links_path)).astype(int)
        if joint_colors_path is None: self.joint_colors = np.ones((self.data.shape[0], 3))
        else: self.joint_colors = np.load(os.path.join(config['project_directory'],joint_colors_path))
        if link_colors_path is None: self.link_colors = np.ones((len(self.links)*2, 3))
        else: self.link_colors = np.repeat(np.load(os.path.join(config['project_directory'],link_colors_path)),2,axis=0)
        ...
        self.joints = Markers(scaling=self.scaling, spherical=True, antialias=0, parent=self.view.scene)
        self.links = Line(width=self.link_width, connect='segments', method='gl', parent=self.view.scene)
        
        if floor_bounds is not None:
            xs = np.arange(floor_bounds[0], floor_bounds[1]+floor_spacing, floor_spacing)
            ys = np.arange(floor_bounds[2], floor_bounds[3]+floor_spacing, floor_spacing)
            if len(xs)>1 and len(ys)>1:
                pos = np.vstack([
                    np.stack([
                        np.stack([xs, ys.min()*np.ones_like(xs), floor_height*np.ones_like(xs)],axis=1),
                        np.stack([xs, ys.max()*np.ones_like(xs), floor_height*np.ones_like(xs)],axis=1)
                    ],axis=1).reshape(-1,3),
                    np.stack([
                        np.stack([xs.min()*np.ones_like(ys), ys, floor_height*np.ones_like(ys)],axis=1),
                        np.stack([xs.max()*np.ones_like(ys), ys, floor_height*np.ones_like(ys)],axis=1)
                    ],axis=1).reshape(-1,3)])

                self.floor = Line(
                    pos, width=.5, color=(1,1,1,0.5), connect='segments', 
