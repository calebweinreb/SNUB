...
class ScatterPanel(Panel, HeaderMixin):
    ...
    def __init__(self, config, selected_intervals, data_path=None, name='',
                 ...
                 selection_intersection_threshold=0.5, variable_labels=[], **kwargs):
    ...
    def update_scatter(self):
        if self.current_variable_label in self.variable_labels:
            x = self.data[:,2+self.variable_labels.index(self.current_variable_label)]
            if self.sort_nodes_by_variable: self.plot_order = np.argsort(x)[::-1]
            else: self.plot_order = np.arange(len(x))
            x = np.clip((x - self.vmin) / (self.vmax - self.vmin), 0, 1)[self.plot_order]
    ...
    def context_menu(self, event):
        ...
        if self.variable_menu.isVisible(): add_menu_item('Hide variables menu', self.hide_variable_menu)
        else: add_menu_item('Show variables menu', self.show_variable_menu)

        # get enriched variables (only available is nodes are selected)
        label = add_menu_item('Sort variables by enrichment', self.get_enriched_variables)
        if self.is_selected.sum()==0: label.setStyleSheet("QLabel { color: rgb(120,120,120); }")
        label = add_menu_item('Restore original variable order', self.show_variable_menu)
        contextMenu.addSeparator()

        # toggle whether to plot high-variable-val nodes on top
        checkbox = add_menu_item('Plot high values on top', self.toggle_sort_by_color_value, item_type='checkbox')
        if self.sort_nodes_by_variable: checkbox.setChecked(True)
        ...
        else: add_menu_item('Show marker trail',partial(self.toggle_marker_trail,True))
    ...
    def show_variable_menu(self, *args, variable_order=None):
        ...
        if variable_order is None: variable_order = self.variable_labels
        for name in variable_order: self.variable_menu.addItem(name)
        self.variable_menu.show()

    def get_enriched_variables(self):
        if self.is_selected.sum() > 0 and len(self.variable_labels)>0:
            variables_zscore = (self.data[:,2:] - self.data[:,2:].mean(0))/(np.std(self.data[:,2:],axis=0)+self.eps)
    ...
    def toggle_sort_by_color_value(self, check_state):
        if check_state == 0: self.sort_nodes_by_variable = False
        else: self.sort_nodes_by_variable = True
        self.update_scatter()
