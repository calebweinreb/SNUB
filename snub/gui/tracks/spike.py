...
from snub.gui.tracks import TracePlot, TrackGroup, Heatmap
...
class SpikePlot(Heatmap):
    def __init__(
        ...
        **kwargs
        ...
        self.cmap = (
            cmapy.cmap(self.colormap).squeeze()[:, ::-1].astype(np.float32) / 255
        )
        self.canvas = SceneCanvas(
            self, keys="interactive", bgcolor=self.cmap[0], show=True
        )
        ...
        self.lines = Line(
            pos=line_verts.reshape(-1, 2),
            color=np.clip(self.cmap[0] + 0.1, 0, 1),
            method="gl",
            width=0.5,
            connect="segments",
        )
    ...
    def update_current_range(self, current_range):
        ...
        if (self.current_range[1] - self.current_range[0]) >= self.heatmap_range:
            ...
            self.viewbox.camera.set_range(
                x=self.current_range, y=self.get_ylim(), margin=1e-10
            )
            bgcolor = (
                self.cmap[0]
            ...
            self.lines.set_data(color=np.clip(bgcolor + 0.1, 0, 1))

    def spike_coordinates(self):
        ycoords = self.max_label - np.argsort(self.row_order)[self.spike_labels] + 0.5
    ...
    def spike_colors(self):
        ...
        cols = np.around(
            (self.spike_times - self.intervals[0, 0]) / self.min_step
        ).astype(int)
        colors = (
            image_data[rows, np.clip(cols, 0, image_data.shape[1] - 1)].astype(
                np.float32
            )
            / 255
        )
...
class HeadedSpikePlot(TrackGroup):
    def __init__(self, config, selected_intervals, **kwargs):
        ...
        super().__init__(
            config, tracks={"spikeplot": spikeplot}, track_order=["spikeplot"], **kwargs
        )
...
class SpikePlotTraceGroup(TrackGroup):
    def __init__(
        ...
        **kwargs
        ...
        spikeplot = SpikePlot(
            config, selected_intervals, height_ratio=heatmap_height_ratio, **kwargs
        )

        x = spikeplot.intervals.mean(1)
        trace_data = {
            l: np.vstack((x, d)).T for l, d in zip(spikeplot.labels, spikeplot.data)
        }
        trace = TracePlot(
            config, height_ratio=trace_height_ratio, data=trace_data, **kwargs
        )

        super().__init__(
            config,
            tracks={"trace": trace, "spikeplot": spikeplot},
            track_order=["trace", "spikeplot"],
